...
### 🗓️ 2026-05-02: Phase 31 — `/settings/general` Per-Staff Dashboard Locale
- **Problem**: The `/dashboard/settings/general` page was a stub. There was no way for a staff member to pick the language of their own dashboard, and the only locale signal on the dashboard was the public `NEXT_LOCALE` cookie — meaning Staff A picking EN on the public navbar would force Staff B (different account, same browser) to inherit EN on `/dashboard`. Worse: there was no cross-device persistence — Staff A on a new device would default to PT instead of recovering their preference.
- **Invariant (now enforced)**: dashboard locale is **per staff user**, durable across devices (DB) and instant per browser (cookie). Public site language and dashboard language are independent.
- **Migration (Supabase MCP `apply_migration` → `profiles_locale_nullable_with_check`)**: `profiles.locale` was already a `text NOT NULL DEFAULT 'es'` column. Migrated to `text NULL` (no default) + `CHECK (locale IS NULL OR locale IN ('es','pt','en'))`. NULL means "no explicit preference yet" → proxy falls back to `Accept-Language`. Verified via `information_schema.columns` and `pg_constraint`.
- [DONE] `infrastructure/db/schema/organizations.ts` — `profiles.locale` mirrored as nullable `text('locale')`. No `_COLS` selector currently includes `locale` (`resolveTenantOrgId` only reads `role`+`isActive`, `resolvePublicSessionUser` only reads `fullName`+`avatarUrl`+`isActive`); the new `setDashboardLocaleAction` only WRITES it, and the login action explicitly selects `{ isActive, locale }` to hydrate the cookie. Zero `SELECT *`.
- [DONE] `proxy.ts` — new `detectDashboardLocale()` resolver. Priority on `/dashboard` and `/admin`: **`DASHBOARD_LOCALE` cookie → `NEXT_LOCALE` cookie → `Accept-Language` → `pt`**. Public segment unchanged (`detectLocale` still drives `x-locale` outside of dashboard). Auto-seed of `NEXT_LOCALE` is now scoped to non-dashboard routes only, so the dashboard's staff-scoped preference can never bleed into the consumer cookie.
- [DONE] `app/(auth)/login/actions.ts` — staff branch now selects `profiles.locale` along with `isActive`. After confirming the staff is active and BEFORE `redirect()`, if `locale` is non-null and ∈ {es,pt,en}, the action writes the `DASHBOARD_LOCALE` cookie with the same flags as `NEXT_LOCALE` (`path:'/'`, `sameSite:'lax'`, `secure` in prod, `maxAge` 1 year). NULL = no preference → no cookie write (proxy falls back). This makes "Staff A entering on a new browser/incognito → recovers EN automatically from DB" work without any client-side hydration.
- [DONE] `app/(public)/actions.ts → signOutAction` — `cookieStore.delete('DASHBOARD_LOCALE')` is now executed after `supabase.auth.signOut()` and before `redirect('/')`. Two-staff-one-browser scenario: A logs out → cookie gone → B's login replays its own DB-persisted preference (or falls back). Public cookie `NEXT_LOCALE` is intentionally NOT cleared (consumers staying on the public landing keep their language).
- [DONE] `app/(dashboard)/dashboard/settings/general/actions.ts` (NEW) — `setDashboardLocaleAction(raw)`. Zod `z.enum(['es','pt','en'])`, `resolveTenantOrgId(['owner','staff','super_admin'])` (any staff role can change THEIR own preference), Drizzle UPDATE with tenant isolation `WHERE id = user.id AND organization_id = orgId`, mirrors into the `DASHBOARD_LOCALE` cookie, then `revalidatePath('/dashboard', 'layout')`. Returns `Result<{ locale }>` per `STANDARDS.md`.
- [DONE] `app/(dashboard)/dashboard/settings/general/_components/DashboardLanguageSelector.tsx` (NEW) — client component. Native `<select>` (44px min height, `rounded-2xl border-stone-200`, accent-blue floating label sitting on the input border, helper text below) wrapped in `useTransition` for auto-save. Optimistic state with rollback on failure, Sonner `toast.success`/`toast.error`, `disabled` + `Loader2` spinner during pending. Three options in PT/ES/EN order. Mobile-first, touch target ≥44px, 60-30-10 palette (white surface, stone text, sky-600 accent).
- [DONE] `app/(dashboard)/dashboard/settings/general/page.tsx` — Server Component. Reads `x-locale` from async `headers()`, narrows to `Locale`, looks up `settings.general` translation block (direct JSON imports, same pattern as `ViewSwitcher.tsx`), renders the page header (Cormorant title + stone-400 description) and a single white card containing a small-caps `Geral` section heading and the selector. Section pattern matches `/settings/brand/_components/BrandDetailsSection.tsx` exactly (`text-xs font-medium text-stone-400 uppercase tracking-widest` + `bg-white rounded-2xl border border-stone-100 shadow-sm`).
- [DONE] `messages/{es,pt,en}.json` — new `settings.general.*` namespace with canonical keys mirrored across all three files: `title`, `description`, `sectionLabel`, `languageLabel`, `languageHelper`, `optionPt`, `optionEs`, `optionEn`, `toastSuccess`, `toastError`. Option labels stay in their native language regardless of UI locale (Português / Español / English) — same convention as the public `LanguageSwitcher`.
- **Verification**:
  - **Migration**: `apply_migration` returned `success`. `information_schema.columns` confirms `is_nullable='YES'`, `column_default=null`. `pg_constraint` confirms `profiles_locale_check` is `CHECK (((locale IS NULL) OR (locale = ANY (ARRAY['es','pt','en']))))`.
  - **Types**: `pnpm check-types` (next typegen + tsc --noEmit) → **EXIT=0, 0 errors** under strict mode.
  - **Tenant + RBAC**: action uses `resolveTenantOrgId(['owner','staff','super_admin'])` so a customer who somehow lands on the route still gets the layout-level `/me` redirect (Phase 29 gate); the action would also reject with `NOT_MEMBER` even if the gate were bypassed.
- **Threat model**: (1) Staff A and Staff B sharing a browser see independent dashboards: A logs in EN → cookie written → A logs out → cookie deleted → B logs in (no preference) → no cookie written → proxy falls back to `Accept-Language` → B's own dashboard locale. (2) Cross-device durability: A signs in from another laptop → no `DASHBOARD_LOCALE` cookie locally → login action reads `profiles.locale='en'` → writes the cookie → dashboard renders in EN on first paint. (3) Public site untouched: `/` and `/book` still resolve via `NEXT_LOCALE`/`Accept-Language` (the dashboard cookie is not consulted), and `setDashboardLocaleAction` does NOT write `NEXT_LOCALE`.
- **Files created**: 2 (`general/actions.ts`, `general/_components/DashboardLanguageSelector.tsx`).
- **Files modified**: 6 (`infrastructure/db/schema/organizations.ts`, `proxy.ts`, `app/(auth)/login/actions.ts`, `app/(public)/actions.ts`, `app/(dashboard)/dashboard/settings/general/page.tsx`, `messages/{es,pt,en}.json`).
- **Follow-ups** (not blocking):
  - Mirror `LanguageSwitcher` widget directly inside the dashboard top nav so language switching is a one-click affordance (today it lives only at `/settings/general`).
