...
interface SlotActionModalProps {
  ...
  slotTime?:       string;         // "HH:MM" — undefined when opened from month view
  locale:          string;
  onSchedule:      (time?: string) => void;
  isBusinessHour?: boolean;        // only applies when slotTime is defined
}

export function SlotActionModal({ open, onClose, date, slotTime, locale, onSchedule, isBusinessHour = true }: SlotActionModalProps) {
  const [view, setView] = useState<View>('choose');
  const t = useMessages(locale);

  const handleClose    = () => { setView('choose'); onClose(); };
  const handleSchedule = () => { onSchedule(slotTime); handleClose(); };

  // When slotTime is undefined (month view): always show both options
  const showBlock      = !slotTime || isBusinessHour;
  const showOutsideNote = !!slotTime && !isBusinessHour;
      ...
      <Dialog.Portal>
        ...
        >
          <AnimatedShell
            ...
            date={date} slotTime={slotTime} t={t}
            showBlock={showBlock} showOutsideNote={showOutsideNote}
...
function AnimatedShell({ view, setView, date, slotTime, t, showBlock, showOutsideNote, onClose, onSchedule }: {
  view: View; setView: (v: View) => void;
  date: string; slotTime?: string;
  t: CalendarMessages; showBlock: boolean; showOutsideNote: boolean;
  ...
  return (
    <div ref={ref} className="rounded-xl bg-white/85 backdrop-blur-xl border border-stone-200/60 shadow-2xl overflow-hidden">
      ...
      {slotTime && <p className="px-5 pb-2 text-xs text-stone-400 tabular-nums">{slotTime} — {date}</p>}
