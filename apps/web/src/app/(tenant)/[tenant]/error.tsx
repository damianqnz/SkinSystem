...
/**
 * @file (tenant)/[tenant]/error.tsx
 * @description Error boundary for the specialist's public site. Catches render
 *              errors in tenant pages/children — `ConsumerShell` (the group
 *              layout) still provides `<html>`/`<body>` + `globals.css`. Uses an
 *              inline dictionary rather than `useTranslations`: the consumer
 *              bundle ships a narrowed intl set that does not include `errors`,
 *              and the provider itself may be what crashed. The card keeps a solid
 *              spa surface so it stays readable over any brand theme (incl. dark).
 *              Locale is read from `<html lang>` on the client.
 */
