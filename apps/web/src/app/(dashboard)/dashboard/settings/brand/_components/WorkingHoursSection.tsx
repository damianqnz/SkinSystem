...
// PT day names. Week Mon→Sun (1,2,3,4,5,6,0)
const DAYS = [
...
function normalizeTime(t: string): string {
  // PostgreSQL time type returns "HH:MM:SS" — strip seconds so <input type="time"> works
...
function buildRules(initial: DayRule[]): DayRule[] {
  return DAYS.map(({ dow }) => {
    ...
    return {
      ...found,
      openTime:  normalizeTime(found.openTime),
      closeTime: normalizeTime(found.closeTime),
    };
  });
}
  ...
  return (
    <section id="horario" className="space-y-3">
      ...
      <p className="text-xs text-stone-400">
        Destaque quando o seu negócio abre e fecha na sua página de reservas.
      </p>

      <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
        {rules.map((rule, idx) => {
          const label = DAYS.find((d) => d.dow === rule.dayOfWeek)?.label ?? '';
          return (
            <div
              key={rule.dayOfWeek}
            ...
            >
              ...
              <Switch.Root
                ...
                className="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent
                           bg-stone-200 transition-colors data-[state=checked]:bg-amber-400
              ...
              {/* Label */}
              <span className={`w-32 text-sm flex-shrink-0 ${rule.isActive ? 'text-stone-800' : 'text-stone-400'}`}>
                {label}
              </span>
              ...
              {rule.isActive ? (
                ...
                <span className="ml-auto text-xs text-stone-400 font-medium uppercase tracking-wide">Fechado</span>
              )}
            </div>
          );
        })}
