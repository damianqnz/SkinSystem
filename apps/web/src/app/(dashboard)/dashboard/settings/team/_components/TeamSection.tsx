...
function Avatar({ name, url, size = 'md' }: { name: string | null; url: string | null; size?: 'md' | 'sm' }) {
  ...
  return (
    <div className={`${dim} rounded-full bg-amber-100 text-amber-700 font-medium flex items-center justify-center flex-shrink-0`}>
      {initials}
    </div>
  );
}
...
function RoleBadge({ role }: { role: 'super_admin' | 'owner' | 'staff' }) {
  return role === 'owner'
    ? <span className="inline-flex items-center gap-1 text-[10px] bg-amber-50 text-amber-700 rounded px-1.5 py-0.5 font-medium"><ShieldCheck size={10} />Proprietário</span>
    : <span className="inline-flex items-center gap-1 text-[10px] bg-stone-100 text-stone-500 rounded px-1.5 py-0.5 font-medium"><User size={10} />Staff</span>;
}
    ...
    <div className={`flex items-center gap-3 px-5 py-3.5 ${!member.isActive ? 'opacity-50' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-stone-800 truncate">
          ...
          {isSelf && <span className="ml-1.5 text-[10px] text-stone-400">(você)</span>}
        </p>
        <div className="flex items-center gap-2 mt-0.5">
          <RoleBadge role={member.role} />
          {!member.isActive && <span className="text-[10px] text-rose-500 font-medium">Inativo</span>}
        </div>
      </div>
            ...
            >
              {member.role === 'staff' ? (
                <DropdownMenu.Item
                  ...
                  onSelect={() => onChangeRole(member.id, 'owner')}
                >
                ...
                <DropdownMenu.Item
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-stone-700 hover:bg-stone-50 cursor-pointer outline-none"
                  onSelect={() => onChangeRole(member.id, 'staff')}
                >
              ...
              <DropdownMenu.Item
                className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer outline-none ${
                  member.isActive ? 'text-rose-600 hover:bg-rose-50' : 'text-emerald-700 hover:bg-emerald-50'
                }`}
...
function InvitationRow({ inv, onCancel }: { inv: PendingInvitation; onCancel: (id: string) => void }) {
    ...
    <div className="flex items-center gap-3 px-5 py-3.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {expired
            ? <span className="text-[10px] text-rose-500 font-medium flex items-center gap-0.5"><Clock size={9} />Expirado</span>
...
export function TeamSection({ initial, currentUserId }: Props) {
  ...
  function handleCancelInvite(invitationId: string) {
    ...
    startTransition(async () => {
      ...
      if (res.error) {
        toast.error(res.error.message);
        setInvitations(initial.invitations);
      } else {
  ...
  return (
    <section id="equipa" className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
        {members.length === 0 ? (
          <p className="text-sm text-stone-400 text-center py-8">Sem membros ainda.</p>
        ) : (
          members.map((m, idx) => (
          ...
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
            {invitations.map((inv, idx) => (
              <div key={inv.id} className={idx < invitations.length - 1 ? 'border-b border-stone-50' : ''}>
                <InvitationRow inv={inv} onCancel={handleCancelInvite} />
              </div>
            ))}
          </div>
        </>
      ...
      {showInvite ? (
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5 space-y-4">
          <p className="text-sm font-medium text-stone-800">Convidar membro</p>
