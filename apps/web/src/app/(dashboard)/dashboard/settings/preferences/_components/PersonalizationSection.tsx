...
const LANG_OPTIONS = ['pt', 'es', 'en'] as const;

export function PersonalizationSection({ initial }: Props) {
  const t    = useTranslations('dashboard.settings.preferences.personalization');
  const tGen = useTranslations('dashboard.settings.general');
            ...
            <div>
              ...
              <select value={preferredLanguage} onChange={(e) => setPreferredLanguage(e.target.value)} className={selectCls}>
                {LANG_OPTIONS.map((code) => (
                  <option key={code} value={code}>{langLabels[code]}</option>
                ))}
              </select>
            </div>
            <div>
