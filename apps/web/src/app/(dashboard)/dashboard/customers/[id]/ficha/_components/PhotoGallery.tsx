...
function BeforeAfterSlider({ before, after }: { before: string; after: string }) {
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden rounded-sm cursor-col-resize select-none" style={{ aspectRatio: '3/4' }}
      ...
      <span className="absolute top-2 left-2 font-sans text-[9px] uppercase tracking-widest text-white/90 bg-black/35 px-2 py-0.5 rounded-sm pointer-events-none">Antes</span>
      <span className="absolute top-2 right-2 font-sans text-[9px] uppercase tracking-widest text-white/90 bg-black/35 px-2 py-0.5 rounded-sm pointer-events-none">Después</span>
    </div>
  );
}
...
function SessionBlock({ session, locale }: { session: PhotoSession; locale: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {before && after ? <BeforeAfterSlider before={before} after={after} /> : (
...
export function PhotoGallery({ sessions, locale }: Props) {
  ...
  return (
    <div className="space-y-8">
      {sessions.filter(s => s.photos.length > 0).map(s => <SessionBlock key={s.sessionId} session={s} locale={locale} />)}
    </div>
  );
}
