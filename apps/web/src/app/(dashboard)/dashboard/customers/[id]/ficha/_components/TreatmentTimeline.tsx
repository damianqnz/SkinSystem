        ...
        return (
          <motion.li key={entry.appointmentId} variants={item} className="relative flex gap-4 pb-8 last:pb-0">
            <div className="flex-shrink-0 w-10 flex items-start justify-center pt-0.5 z-10">
              <div className="w-[10px] h-[10px] rounded-full mt-[5px] ring-2 ring-[var(--color-spa-bg)]" style={{ backgroundColor: meta.color }} />
            </div>
            <div className="flex-1 bg-white/50 backdrop-blur-sm border border-[var(--color-spa-border)] rounded-sm p-4 space-y-3 hover:bg-white/80 transition-colors duration-150">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div className="space-y-0.5">
                  <p className="font-serif text-base font-light text-[var(--color-spa-stone)] leading-tight">{entry.serviceName}</p>
                  <p className="font-sans text-xs text-[var(--color-spa-muted)]">{date} · {time}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="font-sans text-sm text-[var(--color-spa-muted)] tabular-nums">{formatCents(entry.totalCents)}</span>
              ...
              {entry.clinicalSessionId && (
                <div className="pt-2 border-t border-[var(--color-spa-border)] space-y-3">
                  <p className="font-sans text-[10px] uppercase tracking-[0.15em] text-[var(--color-spa-muted)]">Notas Clínicas</p>
                  {entry.professionalNotes && <RevealField value={entry.professionalNotes} label="Notas del profesional" />}
                  {entry.skinReactionNotes && <RevealField value={entry.skinReactionNotes} label="Reacción cutánea" variant="danger" />}
                </div>
              )}
            </div>
          </motion.li>
