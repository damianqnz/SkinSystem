...
interface EventDetailSheetProps {
  ...
  /** Hint shown immediately while the lazy fetch resolves */
  preview?: { customerName: string; serviceColor: string | null } | null;
  locale?: string;
  onMutated: () => void;
}

const STATUS_COPY: Record<string, { es: string; pt: string; en: string; tone: 'amber' | 'emerald' | 'stone' }> = {
...
export function EventDetailSheet({
  ...
  // Lazy fetch when opened with a fresh id
  useEffect(() => {
  ...
  const stub = (label: string) =>
    toast(`${label} · em breve`, {
      description: 'Esta ação chega na próxima fase.',
    });

  const status = data?.status ?? 'pending';
  const statusInfo = STATUS_COPY[status]!;
  const statusLabel = statusInfo[locale === 'pt' ? 'pt' : locale === 'en' ? 'en' : 'es'];
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-spa-border shrink-0">
            <Dialog.Title
              className="text-[12px] uppercase tracking-[0.16em] text-spa-muted"
          ...
          <div className="px-5 py-5 border-b border-spa-border shrink-0">
            <div className="flex items-start gap-3">
              <span
                ...
                style={{ backgroundColor: data?.serviceColor ?? preview?.serviceColor ?? '#D4AF37' }}
              />
              <div className="flex-1 min-w-0">
                <h2
                  className="text-2xl font-light leading-tight tracking-wide text-(--color-spa-stone) uppercase truncate"
                  style={{ fontFamily: 'var(--font-serif)' }}
                >
                  {data ? resolveServiceName(data.serviceName, locale) : (loading ? '…' : '—')}
                </h2>
                {data && (
                  <p className="mt-1 text-[12px] text-spa-muted tabular-nums"
                     style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <Tabs.Root defaultValue="detalhes" className="flex-1 flex flex-col min-h-0">
            <Tabs.List className="flex items-center gap-1 px-5 border-b border-spa-border shrink-0">
              <TabTrigger value="detalhes" label="Detalhes" />
              <TabTrigger value="historico" label="Histórico" disabled />
            </Tabs.List>

            {/* Detalhes panel */}
            <Tabs.Content value="detalhes" className="flex-1 overflow-y-auto no-scrollbar px-5 py-5 space-y-5">
              {/* Time */}
              <Section icon={<Clock size={13} strokeWidth={1.5} />} label="Quando">
                {data ? (
                  <p className="text-[13px] text-(--color-spa-stone) tabular-nums capitalize"
                     style={{ fontFamily: 'var(--font-sans)' }}>
                    {fmtDateLong(data.startAt, locale)} · {fmtTime(data.startAt, locale)} — {fmtTime(data.endAt, locale)}
                  </p>
                ) : <Placeholder />}
              </Section>

              {/* Customer */}
              <Section icon={<User size={13} strokeWidth={1.5} />} label="Cliente">
                {data ? (
                  <div className="flex items-start gap-3">
                    <span
                      className="w-9 h-9 rounded-full bg-stone-100 text-(--color-spa-stone) text-[11px] font-medium flex items-center justify-center shrink-0"
                      style={{ fontFamily: 'var(--font-sans)' }}
                    >
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <p
                        className="text-[14px] leading-snug text-(--color-spa-stone)"
                        style={{ fontFamily: 'var(--font-serif)' }}
                      >
                        {data.customerName}
                      </p>
                      {data.customerEmail && (
                        <p className="flex items-center gap-1.5 text-[12px] text-spa-muted truncate"
                           style={{ fontFamily: 'var(--font-sans)' }}>
                      ...
                      {data.customerPhone && (
                        <p className="flex items-center gap-1.5 text-[12px] text-spa-muted"
                           style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              {/* Staff */}
              <Section icon={<User size={13} strokeWidth={1.5} />} label="Equipa">
                {data ? (
                  <p className="text-[13px] text-(--color-spa-stone)"
                     style={{ fontFamily: 'var(--font-sans)' }}>
                    {data.staffName ?? '—'}
                  </p>
                ) : <Placeholder />}
              ...
              {/* Notes */}
              <Section icon={<Clock size={13} strokeWidth={1.5} />} label="Notas">
                {data
                  ? (
                    <p className="text-[12px] text-(--color-spa-stone) whitespace-pre-wrap"
                       style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              {/* Status */}
              <Section icon={<Clock size={13} strokeWidth={1.5} />} label="Status">
                {data && (
                  <span className={cn(
                    'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-wider border',
            ...
            <Tabs.Content value="historico" className="flex-1 overflow-y-auto px-5 py-5">
              <p className="text-[12px] text-spa-muted text-center mt-6"
                 style={{ fontFamily: 'var(--font-sans)' }}>
                Histórico chega em breve.
              </p>
            </Tabs.Content>
          </Tabs.Root>
...
function TabTrigger({ value, label, disabled }: { value: string; label: string; disabled?: boolean }) {
  return (
    <Tabs.Trigger
      ...
      disabled={disabled}
      className={cn(
        'px-3 py-2 text-[12px] tracking-wide border-b-2 border-transparent transition-colors',
