...
export function SlotActionModal({ open, onClose, slot, locale, onSchedule, isBusinessHour = true }: SlotActionModalProps) {
  const tSlot  = useTranslations('calendar.slot');
      ...
      <Dialog.Portal>
        ...
        >
          <AnimatedShell
            ...
            slot={slot} tSlot={tSlot} tBlock={tBlock}
...
function AnimatedShell({ view, setView, slot, tSlot, tBlock, isBusinessHour, locale, onClose, onSchedule }: {
  ...
  isBusinessHour: boolean; locale: string;
  ...
  const blockT = {
    from:        tBlock('from'),
    to:          tBlock('to'),
    reasonLabel: tBlock('reasonLabel'),
    confirm:     tBlock('confirm'),
    success:     tBlock('success'),
    reason: {
      illness:  tBlock('reason.illness'),
      vacation: tBlock('reason.vacation'),
      training: tBlock('reason.training'),
      other:    tBlock('reason.other'),
    },
  };
    ...
    <div ref={ref} className="rounded-xl bg-white/85 backdrop-blur-xl border border-stone-200/60 shadow-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-5 pb-3">
        <Dialog.Title className="font-cormorant text-xl font-semibold text-stone-800">
          {view === 'choose' ? tSlot('chooseAction') : tBlock('title')}
        </Dialog.Title>
        ...
        <div className="px-5 pb-5 space-y-2.5">
          {!isBusinessHour && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-100">
              ...
              <span className="text-[11px] text-amber-700">{tSlot('outsideNote')}</span>
            </div>
          )}
          {isBusinessHour && (
            <ActionTile icon={<Lock size={20} />} title={tSlot('block')} desc={tSlot('blockDescription')} onClick={() => setView('block')} />
          )}
          <ActionTile icon={<CalendarPlus size={20} />} title={tSlot('schedule')} desc={tSlot('scheduleDescription')} onClick={onSchedule} />
        </div>
      ) : (
        <BlockTimeForm slot={slot} t={blockT} onClose={onClose} />
      )}
    </div>
  );
}
