    ...
    <div className="px-5 pb-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <EditorialDatePicker value={from} onChange={handleFromChange} minDate={minDateIso} />
        </div>
        <div>
          <label className="block text-[11px] font-medium text-stone-500 uppercase tracking-wide mb-1.5">{t('to')}</label>
          <EditorialDatePicker value={to} onChange={handleToChange} minDate={from} />
        </div>
      </div>

      {/* Reason select */}
      <div>
        <label className="block text-[11px] font-medium text-stone-500 uppercase tracking-wide mb-1.5">{tBlock('reasonLabel')}</label>
              ...
              <Select.Viewport className="p-1">
                {REASONS.map((r) => (
                  <Select.Item key={r} value={r} className="flex items-center gap-2 px-3 py-2 text-sm rounded-md cursor-pointer hover:bg-stone-50 outline-none data-[highlighted]:bg-stone-50">
                    ...
                    <Select.ItemText>{tBlock(`reason.${r}`)}</Select.ItemText>
                  </Select.Item>
                ))}
              </Select.Viewport>
            </Select.Content>
