...
interface CatalogIslandProps {
  category:       CategoryWithServices | null;   // null = "Sin categoría" island
...
export function CatalogIsland({
  category, categories, locale, organizationId, onEditCategory,
}: CatalogIslandProps) {
  ...
  const catName    = category ? resolveI18n(category.nameI18n, locale) : 'Sin categoría';
  const catDesc    = category ? resolveI18n(category.descriptionI18n, locale) : '';
  const isOrphan   = category === null;

  // Skip orphan island if empty
  ...
  function openEdit(svc: ServiceRowType) {
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {catDesc && (
              <p className="text-xs text-stone-400 mt-0.5 truncate">{catDesc}</p>
            )}
          </div>
        ...
        {/* Gold separator */}
        <div className="h-px bg-gradient-to-r from-transparent via-amber-200/60 to-transparent mx-5" />
            ...
            >
              {services.length === 0 ? (
                <div className="px-5 py-6 text-center">
                  <p className="text-sm text-stone-400">Esta categoría está vacía.</p>
                </div>
              ) : (
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-stone-50">
                      <th className="py-2 pl-4 pr-3 text-left text-[10px] font-medium text-stone-400 uppercase tracking-widest">Servicio</th>
                      <th className="py-2 px-3 text-left text-[10px] font-medium text-stone-400 uppercase tracking-widest">Precio</th>
                      <th className="py-2 px-3 text-left text-[10px] font-medium text-stone-400 uppercase tracking-widest hidden sm:table-cell">Duración</th>
                      <th className="py-2 px-3 text-left text-[10px] font-medium text-stone-400 uppercase tracking-widest hidden md:table-cell">Depósito</th>
                      <th className="py-2 px-3 text-left text-[10px] font-medium text-stone-400 uppercase tracking-widest hidden sm:table-cell">Estado</th>
