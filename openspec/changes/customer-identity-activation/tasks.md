...
- [x] **D.1** Booking-confirmation CTA in `apps/web/src/app/(tenant)/[tenant]/book/success/page.tsx`: a link to /login?next=/me with copy explaining that signing in with a link turns the booking into an account. It introduces **no identity logic** — activation happens through the B funnel. Render it only when the visitor has no session (design §4.3: the unactivated moment is a guest with no session; showing it to a signed-in customer is noise). Keys `booking.success.activate*` in three locales. Satisfies: Req 4 scenario 2, D6. **I18N-CHK**. **DONE 2026-09-25: `createSupabaseServerClient()` + `getUser()`, CTA rendered only when `!user`; commit `e08c30d`.**
- [x] **D.2** `SetPasswordForm` (client component) under `apps/web/src/app/(account)/me/_components/`, mounted from `me/perfil/page.tsx`: `supabase.auth.updateUser({ password })` from the already-authenticated session via the browser client (auth surface only; never touches `customers`). Zod-validate (minimum length aligned with the Supabase Auth setting, confirmation match). Map errors to translated messages including `weak_password` (Leaked Password Protection, once G is on) and `reauthentication_needed`. Keys `account.me.perfil.password.*`. Confirm `account` is shipped to the client provider (the audit test guards this). Satisfies: design §4.3 residual affordance, Req 5 (password is secondary, only ever set from an authenticated session), Req 8. **I18N-CHK**. **DONE 2026-09-25: `set-password-schema.ts` (pure, unit-tested, min length 6 aligned with the existing `loginSchema`) + `SetPasswordForm.tsx` (browser client only, `htmlFor`/`id` label association, `issue.code === 'custom'` mismatch discriminator); commit `49af551`.**
- [ ] **D.3 [M]** QA: guest booking → success page shows CTA (no session) → /login → OTP → /me; set a password on /me/perfil, sign out, sign in with that password → /me (exercises B's 7a identity-first path); CTA hidden when signed in. After G is enabled, a known-leaked password is rejected with the translated message. **Human-deferral note (2026-09-25): needs a deployed environment (real OTP email delivery, a real Supabase Auth session) that this sdd-apply slice does not have; left unchecked for the human to run.**
- [x] **D.4** Gate commands; **REQ8-CHK**. **DONE 2026-09-25: `pnpm check-types` exit 0, `pnpm lint` exit 0, `pnpm exec turbo run test --force` 153/153, `pnpm build` exit 0; REQ8-CHK 0 matches; `git diff --stat` shows no `proxy.ts` / `i18n/request.ts` change.**
- [x] **D.5 HB** **DONE 2026-09-25: HEARTBEAT.md entry added.**
- [x] **D.6** Rollback boundary: pure UI revert; no data or schema effect. **Confirmed: `git revert 49af551 e08c30d` removes the CTA link, `SetPasswordForm`, and the schema file; no migration, no `customers`/`auth.users` write, no data dependency either direction.**
