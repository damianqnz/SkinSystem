...
| E.2 OTP + Apple | done | `signInWithOtp({ email, options: { emailRedirectTo: \`${origin}/auth/confirm?next=/book\`, shouldCreateUser: true } })`; identical "check your inbox" state on success or a returned `{ error }` except a distinct 429 rate-limit message — and, per the gatekeeper fix below, a retryable `otpError` (`{ status: 'failed' }`, form stays visible) on a THROWN network failure rather than a false "sent"; `signInWithOAuth` generalized to `'google' \| 'apple'`; Apple rendered by mapping `resolveEnabledOAuthProviders({ appleEnabled: process.env.NEXT_PUBLIC_SUPABASE_APPLE_ENABLED })`, ships inert (flag unset). |
| E.3 i18n | done | `booking.auth.*` in pt/es/en: 9 keys dropped (verified zero other consumers via `rg` before deletion), 4 keys added in `6dd3709` (`apple`, `otpCta`, `otpSent` w/ ICU `{email}`, `otpRateLimited`), plus a 5th, `otpError`, added in the gatekeeper round below for the thrown-error state; `emailLabel`/`emailPlaceholder` reused for the OTP field. Parity green across all keys (`messages.test.ts`). |
...
3. **`OtpState` is a 4-arm discriminated union** (`idle | sent | rateLimited | failed`). Anti-enumeration still holds for every outcome Supabase *returns*: success and any non-429 returned `{ error }` resolve to the identical `sent` confirmation, and a 429 resolves to `rateLimited`. The `failed` arm covers only a THROWN (network/unexpected) exception from `signInWithOtp` — not an enumeration signal, since it does not depend on whether the account exists — and renders a retryable `otpError` with the form still visible instead of falsely claiming the link was sent. (Originally shipped as a 3-arm union `idle | sent | rateLimited`; the `failed` arm was added in the gatekeeper round below, after the `gga` pre-commit review caught the misleading "sent"-on-throw.)
...
6. **Gatekeeper round — misleading "sent"-on-throw fixed** (pre-commit `gga` review, working tree — not yet committed by the parent): the `catch` introduced with `bfe6018`'s `try/finally` still let a THROWN `signInWithOtp` failure (e.g. a network drop) fall through to the `sent` state, falsely claiming the link was sent. It now sets a new `{ status: 'failed' }` arm that keeps the form visible and renders `t('otpError')` with `role="alert"`; the password-form error was also given `role="alert"` for consistency. A new `otpError` key was added under `booking.auth` in pt/es/en (tri-locale parity green). No enumeration signal is introduced — a thrown error is independent of whether the account exists; the anti-enumeration `sent` state still applies to success and a returned `{ error }` (except a 429 → `rateLimited`).
...
The gatekeeper fix (working tree, pending a parent commit) adds only the `{ status: 'failed' }` arm plus the `t('otpError')` / `role="alert"` rendering in `Step2Auth.tsx` and one `otpError` key per locale; reverting that commit removes them and returns to the pre-fix "sent"-on-throw behavior — still no data dependency in either direction.
